...
describe("DBSearchService", () => {
  ...
  beforeEach(() => {
    ...
    service = new DBSearchService(productRepo, shopRepo, categoryRepo, brandRepo);
  });
  ...
  describe("searchProducts", () => {
    it("should search products with filters", async () => {
      ...
      const result = await service.searchProducts({ query: "test", shopId: "shop-1", inStock: true });
  ...
  describe("globalSearch", () => {
    it("should search across all entities and aggregate results", async () => {
      ...
      shopRepo.findMany.mockResolvedValue([{ id: "shop-1", name: "Shop 1", created_at: date }]);
      ...
      brandRepo.findMany.mockResolvedValue([{ id: "brand-1", name: "Brand 1" }]);
