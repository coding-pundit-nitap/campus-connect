...
describe("NotificationService", () => {
  ...
  beforeEach(() => {
    ...
    service = new NotificationService(broadcastRepo, notificationRepo);
  });

  describe("publishNotification", () => {
    it("should add job to notification queue", async () => {
      await service.publishNotification("user-1", { title: "Title", message: "Message", type: "INFO" } as any);
  ...
  describe("broadcastNotification", () => {
    it("should add job to notification queue", async () => {
      await service.broadcastNotification({ title: "Broadcast", message: "Broadcast msg", type: "INFO" } as any);
      expect(notificationQueue.add).toHaveBeenCalledWith("broadcast-notification", {
        type: "BROADCAST_NOTIFICATION",
        payload: {
          type: "BROADCAST_NOTIFICATION",
          data: { title: "Broadcast", message: "Broadcast msg", type: "INFO" },
        },
      });
    });
  });
  ...
  describe("getUserNotifications", () => {
    it("should call repository getNotificationsByUserId", async () => {
      ...
      expect(notificationRepo.getNotificationsByUserId).toHaveBeenCalledWith("user-1", { limit: 10, cursor: "cursor-1" });
    });
  });
  ...
  describe("getAllNotificationsWithBroadcasts", () => {
    it("should return merged notifications and broadcasts", async () => {
      ...
      const result = await service.getAllNotificationsWithBroadcasts("user-1", 10);
  ...
  describe("getPaginatedUnreadBroadcasts", () => {
    it("should call repository findUnreadForUser", async () => {
      ...
      expect(broadcastRepo.findUnreadForUser).toHaveBeenCalledWith("user-1", { limit: 10, cursor: "cursor-1" });
    });
  });
  ...
  describe("markNotificationsAsRead", () => {
    it("should call repository markManyAsRead", async () => {
      ...
      expect(notificationRepo.markManyAsRead).toHaveBeenCalledWith("user-1", ["notif-1"]);
    });
  });
  ...
  describe("markBroadcastsAsRead", () => {
    it("should call repository markManyAsReadForUser", async () => {
      ...
      expect(broadcastRepo.markManyAsReadForUser).toHaveBeenCalledWith("user-1", ["broad-1"]);
    });
  });
