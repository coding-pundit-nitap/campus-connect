...
verify_postgres() {
  ...
  docker exec -e PGPASSWORD="$PG_PASSWORD" campus_connect_db \
    psql -U "$PG_USER" -c "CREATE DATABASE ${test_db};" >/dev/null 2>&1 && \
  zcat "$dump_file" | \
    docker exec -i -e PGPASSWORD="$PG_PASSWORD" campus_connect_db \
    pg_restore -U "$PG_USER" -d "$test_db" --no-owner 2>/dev/null && \
  TABLE_COUNT=$(docker exec -e PGPASSWORD="$PG_PASSWORD" campus_connect_db \
...
verify_age() {
  ...
  local backup_epoch
  backup_epoch=$(date -d "${dir_ts}" +%s 2>/dev/null || echo 0)
